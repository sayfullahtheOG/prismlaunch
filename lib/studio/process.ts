...
export function nextInstruction(process: Process): {
  ...
  const how: Record<StageId, string> = {
    ...
    storyboard: "Read SKILL.md's Visual storyboards section and PRISM_METHOD.md §2 and §10. DRAW each shot using required visual.layers: real screenshots or labelled rough subjects, positioned UI regions, text, cursors and timed keyframes. Section titles alone are not boards. Include frame, action, handoff, durationInFrames, transitions and sound. Inspect opening/closing poses and the rough sequence, then prism.submit_storyboard.",
